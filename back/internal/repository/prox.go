...
type ProxyLink struct {
	...
	LinkSourceType int64  `json:"source_type"`
	LinkProxyType  int64  `json:"proxy_type"`
...
func (r *PROXRepositoryLayer) threadUpload(resp *MProxyPreset) {
	for _, item := range *resp.Links {
		...
		case 17:
			...
			if err != nil {
				r.db.Exec(lib_db.TxWrite, QUpdateProxyPresetLinkStatus, 13, "Ошибка соединения", resp.ID, item.LinkID)
				continue
			}
		...
		if err != nil {
			r.db.Exec(lib_db.TxWrite, QUpdateProxyPresetLinkStatus, 13, msg, resp.ID, item.LinkID)
			continue
		}

		if tx, err = r.db.StartTx(lib_db.TxWrite); err != nil {
			r.db.Exec(lib_db.TxWrite, QUpdateProxyPresetLinkStatus, 13, "Ошибка транзакции", resp.ID, item.LinkID)
			continue
		}
		...
		if err = trueTx.Commit(); err != nil {
			r.db.Exec(lib_db.TxWrite, QUpdateProxyPresetLinkStatus, 13, "Ошибка коммита", resp.ID, item.LinkID)
			continue
		}

		r.db.Exec(lib_db.TxWrite, QUpdateProxyPresetLinkStatus, 14, nil, resp.ID, item.LinkID)
	}
}
...
func (r *PROXRepositoryLayer) UploadProxyPreset(resp *MProxyPreset) (int64, error) {
	...
	for lid, item := range *resp.Links {
		if item.LinkSourceType < 15 || item.LinkSourceType > 17 || len(item.LinkPath) < 3 ||
			item.LinkProxyType < 18 || item.LinkProxyType > 20 {
			return 0, errors.New("Перепроверьте указанные источники")
		}

		if item.LinkSourceType == 15 && item.LinkData == "" {
